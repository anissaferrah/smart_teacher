...
def get_presentation_prompt(
    ...
    prompts_map = {
        "en": (
            ...
            "- 4 to 6 sentences of CONCRETE EXPLANATION. English only.\n"
            "- FORBIDDEN: NEVER end with an open rhetorical question "
            "(e.g. 'What are the main objectives...?', 'How does this work?'). "
            "The student is listening passively and cannot answer. Instead, end with "
            "an AFFIRMATIVE sentence summarising the key point or bridging to what's next.\n"
            "- FORBIDDEN: no questions in the narration, unless the slide itself poses "
            "one (in which case quote it verbatim and answer it).\n"
            "- REQUIRED: when the slide names a concept, give a concrete explanation "
            "or example from the course material — do not just announce 'we will see...'.\n\n"
            "MATH NOTATION:\n"
        ...
        "fr": (
            ...
            "- 4 à 6 phrases d'EXPLICATION CONCRÈTE. Uniquement en français.\n"
            "- INTERDIT : NE TERMINE JAMAIS par une question rhétorique ouverte "
            "(ex: 'Quels sont les principaux objectifs...?', 'Comment cela fonctionne-t-il ?'). "
            "L'étudiant écoute en mode passif et ne peut pas répondre. Termine au contraire "
            "par UNE PHRASE AFFIRMATIVE qui résume le point clé ou ouvre la suite.\n"
            "- INTERDIT : pas de question dans la narration, sauf si la slide elle-même "
            "pose une question (auquel cas tu la répètes telle quelle pour y répondre ensuite).\n"
            "- OBLIGATOIRE : si la slide cite un concept, donne-en une explication ou un "
            "exemple concret tiré du cours, ne te contente pas d'annoncer 'nous allons voir...'.\n\n"
...
class Brain:
    def __init__(self):
        ...
        # Provider selection : OpenAI > Groq > Ollama. ``self.client`` is
        # the high-quality cloud-LLM handle (OpenAI by default, repointed
        # at Groq when OpenAI is disabled but Groq is configured). All
        # downstream call-sites in this file use ``self.client.chat.
        # completions.create(model=self._client_model, ...)`` so the same
        # code path works for both providers thanks to Groq's OpenAI-
        # compatible REST surface.
        self._client_model: str = Config.GPT_MODEL          # filled below
        ...
        if Config.DISABLE_OPENAI and getattr(Config, "GROQ_API_KEY", None):
            ...
            try:
                ...
                log.info(
                    "🟢 Brain : Groq activé (OpenAI désactivé) | model=%s base=%s",
                    self._client_model, Config.GROQ_BASE_URL,
                )
        ...
        # Chaîne LLM : self.client (OpenAI ou Groq) → Ollama (mistral).
    ...
    def ask(
        ...
        if self.fallback and self.fallback.available:
            ...
            # max_tokens=700 : a structured pedagogical answer (definition +
            # rephrasing + intuition + example) is ~400-500 tokens in
            # French. The previous cap of 250 systematically truncated
            # answers mid-example, leaving the student with just an
            # opening example and no definition. 700 leaves ~200 tokens
            # of headroom and keeps Ollama latency reasonable.
