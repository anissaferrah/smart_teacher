...
class PlannerAgent:
    ...
    async def __call__(self, state: TutorState) -> dict[str, Any]:
        ...
        log.info(
            "🔍 planner PROMPT (lang=%s, slide_chars=%d) ===\n%s\n=== END",
            lang, len(state.get("last_slide_content") or ""),
            prompt[:3000] + ("…[truncated]" if len(prompt) > 3000 else ""),
        )
        raw, _duration = self.brain.ask(prompt, reply_language=lang)
        log.info("🔍 planner LLM RAW (%d chars):\n%s",
                 len(raw or ""), (raw or "")[:1500])
        ...
        for i, idea in enumerate(plan.ideas):
            log.info(
                "🔍   plan[%d] type=%s depth=%s : %r",
                i, getattr(idea, "type", "?"), getattr(idea, "depth", "?"),
                (getattr(idea, "content_brief", "") or "")[:120],
            )
        return {
            "plan": plan,
            "main_concept_hint": main_concept,
