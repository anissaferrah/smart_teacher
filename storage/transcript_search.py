...
class TranscriptSearcher:
    ...
    def index(self, entry: TranscriptEntry) -> bool:
        ...
        if self._use_es:
            try:
                ...
                log.info(
                    "🔵 ES INDEX | index=%s role=%s lang=%s | text_len=%d | took=%.0fms",
                ...
                log.warning("🔵 ES INDEX FAIL (%s) → fallback memory", e)
                self._use_es = False
        ...
        log.debug(
            "🧠 MEMORY INDEX | role=%s lang=%s | total_in_mem=%d/5000",
    ...
    def _es_search(self, query, language, course_id, role, limit) -> list[dict]:
        ...
        try:
            _t0 = time.time()
            log.info(
                "🔵 ES SEARCH START | index=%s | q='%s' lang=%s course=%s role=%s limit=%d filters=%d",
                ES_INDEX, query[:60], language or "*", course_id or "*", role or "*", limit, len(filters),
            )
            resp = self._session.post(
            ...
            r = resp.json()
            log.info(
                "🔵 ES SEARCH DONE | hits=%d/%d | took_es=%dms total=%.0fms",
                len(r.get("hits", {}).get("hits", [])),
                r.get("hits", {}).get("total", {}).get("value", 0) if isinstance(r.get("hits", {}).get("total"), dict) else 0,
                int(r.get("took") or 0),
                (time.time() - _t0) * 1000,
            )
            results = []
