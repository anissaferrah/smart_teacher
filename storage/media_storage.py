...
class MediaStorage:
    ...
    def upload_bytes(
        ...
        log.info(
            f"💾 STORAGE upload START | backend={backend} object={object_name} "
            f"size={len(data)}B ({size_kb:.1f}KB) mime={content_type}"
        )
        if self._use_minio:
        ...
        throughput = (size_kb / elapsed) if elapsed > 0 else 0.0
        log.info(
            f"💾 STORAGE upload DONE | backend={backend} object={object_name} | "
            f"took={elapsed*1000:.0f}ms throughput={throughput:.0f}KB/s | url={url[:80]}"
        )
        return url
    ...
    def get_bytes(self, object_name: str) -> Optional[bytes]:
        ...
        if self._use_minio:
            try:
                ...
                response.close()
                log.info(
                    f"💾 STORAGE get_bytes HIT | backend=minio object={object_name} | "
                    f"size={len(data)}B took={(time.time()-t0)*1000:.0f}ms"
                )
                return data
            except Exception as exc:
                log.warning(f"💾 STORAGE get_bytes MISS minio | object={object_name} err={exc}")
                return None
        ...
        if path.exists():
            ...
            log.info(
                f"💾 STORAGE get_bytes HIT | backend=local object={object_name} | "
                f"size={len(data)}B took={(time.time()-t0)*1000:.0f}ms"
            )
            return data
        log.info(f"💾 STORAGE get_bytes MISS | backend=local object={object_name} (not found)")
        return None
