...
LLM_SUMMARY      = Config.GPT_MODEL
LLM_ANSWER       = Config.GPT_MODEL
...
class MultiModalRAG:
    ...
    def generate_final_answer(
        ...
        try:
            ...
            if not self._openai_disabled_reason:
                llm = ChatOpenAI(
                    model=LLM_ANSWER, temperature=0.4, max_tokens=500, max_retries=0,
            ...
            try:
                ...
                fallback_llm = LocalLLMFallback(model=Config.OLLAMA_TEXT_MODEL)
                if fallback_llm.available:
                    log.info(f"🖥️ Ollama fallback ({fallback_llm.base_url}) with {Config.OLLAMA_TEXT_MODEL}...")
    ...
    async def generate_final_answer_stream(
        ...
        try:
            ...
            try:
                ...
                if fallback_llm.available:
                    log.info(f"🖥️ Utilizing Ollama ({fallback_llm.base_url}) with {Config.OLLAMA_TEXT_MODEL} model...")
    ...
    def generate_quiz(
        ...
        try:
            ...
            if not self._openai_disabled_reason:
                llm = ChatOpenAI(
                    model=LLM_ANSWER, temperature=0.35, max_tokens=800, max_retries=0,
                    base_url=Config.OPENAI_BASE_URL, api_key=Config.OPENAI_API_KEY,
                )
                response = llm.invoke(self._build_chat_messages(system_prompt, history, user_content))
            ...
            try:
                ...
                fallback_llm = LocalLLMFallback(model=Config.OLLAMA_TEXT_MODEL)
                if fallback_llm.available:
                    log.info(f"🖥️ Ollama quiz fallback ({fallback_llm.base_url}) with {Config.OLLAMA_TEXT_MODEL}...")
    ...
    def _get_or_create_summary(self, text: str, chapter_context: str) -> str:
        ...
        try:
            llm = ChatOpenAI(
                model=LLM_SUMMARY, temperature=0.0, max_tokens=300, max_retries=0,
                base_url=Config.OPENAI_BASE_URL, api_key=Config.OPENAI_API_KEY,
            )
