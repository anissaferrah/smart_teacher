...
# ── Server-side auth gate for protected HTML pages ────────────────────
#
# The previous setup relied on a client-side gate inside ``index.html``
# (a small JS block that redirects to login if no token is in
# localStorage). That works for normal users but is trivially bypassed
# by disabling JavaScript or fetching the file directly. The server-
# side middleware below blocks access to every ``.html`` page in
# ``static/`` UNLESS one of these is true :
#
#   1. The path is the login page itself.
#   2. The request carries a valid JWT — either in the
#      ``smart_teacher_token`` cookie (set by /auth/login) OR in the
#      ``Authorization: Bearer <token>`` header.
#
# Static assets (CSS, JS, fonts, images) are NOT gated — they're
# referenced by login.html and other public-shell needs. Only HTML
# pages are protected, which is what defines a "page" in our app.

_PUBLIC_HTML_PAGES = {
    "/static/login.html",   # login page itself
}


@app.middleware("http")
async def static_html_auth_gate(request, call_next):
    """Server-side enforcement of the auth gate for /static/*.html.

    Trivial bypass via "disable JavaScript" or direct URL fetch is
    closed here. The decision is fast (1 cookie read + 1 JWT verify)
    and fails open for non-HTML paths so CSS/JS/images keep working.
    """
    from fastapi.responses import RedirectResponse
    import logging
    _gate_log = logging.getLogger("SmartTeacher.AuthGate")
    path = request.url.path

    # Only gate /static/*.html (and only on GET — POST etc. don't apply)
    if (
        request.method == "GET"
        ...
        # Check cookie first (set on login), then header
        ...
        _gate_log.info(
            "🔐 GATE check | path=%s | token_source=%s present=%s",
            path, token_source, bool(token),
        )
        if not token:
            _gate_log.warning("🔐 GATE REDIRECT | path=%s | reason=no_token → /static/login.html", path)
            return RedirectResponse(url="/static/login.html", status_code=302)
        # Validate the token (signature + expiry). Any failure → redirect.
        try:
            from handlers.auth import decode_access_token
            claims = decode_access_token(token)
            _gate_log.info(
                "🔐 GATE PASS | path=%s | sub=%s role=%s",
                path, str(claims.get("sub", ""))[:8], claims.get("account_level", "?"),
            )
        except Exception as exc:                                          # noqa: BLE001
            _gate_log.warning(
                "🔐 GATE REDIRECT | path=%s | reason=invalid_token (%s) → /static/login.html",
                path, exc,
            )
            return RedirectResponse(url="/static/login.html", status_code=302)
...
from routes.learning_gain       import router as learning_gain_router
from routes.practice            import router as practice_router
from routes.rest                import router as rest_router
from routes.search              import router as search_router
from routes.session             import router as session_router
from routes.student             import router as student_router
from routes.student_state       import router as student_state_router
...
for _router in (
    ...
    learning_gain_router,       # /quiz/{pretest|posttest}/{course_id}/* + /quiz/.../gain
    student_router,             # /student/{id}/* + /student/me/*
    student_state_router,       # /student/me/state, /student/{id}/state (aggregated observability)
...
@app.get("/")
async def root():
    """Root URL : redirect students to the login page so they don't
    land on a raw JSON status response. The auth gate inside
    index.html then forwards them to the app once a token is stored.

    The legacy JSON status payload is moved to /api/status for any
    operator scripts that depended on it.
    """
    from fastapi.responses import RedirectResponse
    return RedirectResponse(url="/static/login.html", status_code=302)
...
async def api_status():
    """Operator JSON status — same payload the legacy ``GET /``
    used to serve. Kept on a separate path so health-check scripts
    don't break after the root redirect."""
    return {
        "status":    "running",
