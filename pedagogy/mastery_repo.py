    ...
    async def upsert_score(
        ...
        try:
            async with AsyncSessionLocal() as db:
                ...
                old_score = existing.score or 0.5
                ...
                log.info(
                    "🔍 mastery UPDATE | idea=%s | %s | correct=%d/%d (confusions=%d) | "
                    "Beta(1,1)+Laplace : (%d+1)/(%d+2) = %.3f (was %.2f, Δ=%+.3f)",
                    str(idea_id)[:30],
                    "CONFUSION" if is_confusion else "CLEAN",
                    _correct, existing.attempts, existing.confusions or 0,
                    _correct, existing.attempts,
                    new_score, old_score, new_score - old_score,
                )
