...
class ContextualThompsonBandit:
    ...
    def select(
        ...
        # Capture every (arm, alpha, beta, draw) for the operator log :
        # this is the single most useful insight when calibrating the
        # bandit — you see WHY the chosen arm was picked over the others.
        _draws: list[tuple[str, int, int, float]] = []
        for action in candidates:
            post = self._post(bucket, action)
            draw = post.sample(self._rng)
            _draws.append((
                action.arm_id,
                int(getattr(post, "alpha", 1)),
                int(getattr(post, "beta", 1)),
                draw,
            ))
            if draw > best_sample:
                best_sample = draw
        ...
        # Sort draws by sampled value so the WINNER is at the top
        _draws.sort(key=lambda x: -x[3])
        log.info(
            "🔍 bandit THOMPSON | bucket=%s | %d arms | winner=%s draw=%.3f",
            bucket.bucket_key, len(candidates), best.arm_id, best_sample,
        )
        for arm_id, a, b, draw in _draws[:5]:   # top 5 only (rest is noise)
            mean = a / (a + b) if (a + b) > 0 else 0.5
            log.info(
                "🔍   arm=%-30s α=%d β=%d  mean=%.2f  sample=%.3f%s",
                arm_id, a, b, mean, draw,
                "  ← chosen" if arm_id == best.arm_id else "",
            )
        return best
