...
def extra_payload_from_state(qa_final: dict | None) -> dict[str, Any]:
    """Extract bandit-related fields + transcript audit from the Q&A
    graph's final state.

    The returned dict is meant to be merged into ``extra_payload`` of
    ``services.learning_log.persist_learning_turn``. It carries three
    classes of information :

      1. **Flat bandit identifiers** (``bandit_strategy``,
         ``bandit_speech_rate``, ``bandit_context``) — the bucket key
         and chosen action. Used by the existing offline RL training
         pipeline (``event_to_episode``) to rebuild ``LoggedEpisode``s
         from Postgres rows.

      2. **Full bandit context** (``bandit_context_full``) — a
         structured dict with the student state at decision time
         (style, pace, mastery, primary_concept). The bucket key alone
         loses the original feature values (e.g. mastery=0.42 → bucket
         "medium"), so we keep the raw values for retrospective
         analyses that don't fit the bucket grid.

      3. **Grounding chunks** (``grounding_chunks``) — the citations the
         responder used to ground its answer. List of compact dicts
         ``{chunk_id, idea_label, source, score}``. Lets a human auditor
         click through every Q&A turn and see which slide chunks
         actually informed each answer (transparency / IRB compliance).

    Returns an empty dict when ``qa_final`` is missing or has no
    actions — meaning the turn wasn't a Q&A grounded turn (e.g. plain
    navigation), so there's no bandit decision to record.
    ...
    # 1. Flat bandit identifiers (existing contract — backward compat).
    for key in ("bandit_strategy", "bandit_speech_rate", "bandit_context"):
        val = payload.get(key)
        if val is not None:
            out[key] = val

    # 2. Full bandit context — student-state snapshot at decision time.
    # We pull the raw feature values from ``qa_final`` rather than the
    # action payload because the responder doesn't echo them onto the
    # action ; they live on the state at the top of the turn.
    if any(out.get(k) for k in ("bandit_strategy", "bandit_context")):
        # Only emit a context_full block when there WAS a bandit decision.
        # On navigation/feedback turns those keys are absent and we skip.
        ctx_full: dict[str, Any] = {}
        for key in (
            "learning_style", "current_idea_id", "language",
            "student_level", "review_mode",
        ):
            if key in qa_final:
                ctx_full[key] = qa_final[key]
        # mastery_map can be huge ; if present, only the avg is useful
        # at the row level (the per-concept detail is in mastery_repo).
        mm = qa_final.get("mastery_map")
        if isinstance(mm, dict) and mm:
            try:
                ctx_full["mastery_avg"] = round(
                    sum(float(v) for v in mm.values()) / len(mm), 3
                )
            except Exception:                                              # noqa: BLE001
                pass
        # confusion (optional — set on confusion_signal turns)
        confusion = qa_final.get("confusion")
        if confusion is not None:
            ctx_full["confusion_score"] = float(getattr(confusion, "score", 0.0) or 0.0)
            ctx_full["confusion_source"] = str(getattr(confusion, "source", "") or "")
        if ctx_full:
            out["bandit_context_full"] = ctx_full

    # 3. Grounding chunks (transcript audit). The responder's
    # ``citations`` list is shaped ``[{chunk_id, idea_label, source,
    # score}, ...]`` (see agentic.qa.responder._parse_qa_response).
    # Cap at 5 to keep the JSON column bounded.
    citations = qa_final.get("citations")
    if isinstance(citations, list) and citations:
        ...
        for c in citations[:5]:
            if not isinstance(c, dict):
                continue
            compact.append({
                "chunk_id": str(c.get("chunk_id", ""))[:40],
                "idea_label": str(c.get("idea_label", ""))[:120],
                "source": str(c.get("source", ""))[:200],
                "score": round(float(c.get("score", 0.0)), 3),
            })
        if compact:
            out["grounding_chunks"] = compact
