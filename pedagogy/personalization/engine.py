...
class PersonalizationEngine:
    ...
    def build_prompt_prefix(ctx: PersonalizationContext, lang: str = "fr") -> str:
        ...
        Les fragments sont entoures de balises ``<<INSTRUCTION_INTERNE>>`` /
        ``<<FIN_INSTRUCTION_INTERNE>>`` (et leurs variantes EN). Pourquoi :
        avant cette enveloppe, le LLM (Ollama 7-8B notamment) confondait
        les directives meta avec la question de l'eleve — par exemple en
        repondant "Voici comment expliquer la strategie pedagogique que
        tu as demandee, avec des mots simples..." au lieu de repondre a
        la vraie question. La regle "ne jamais mentionner ces blocs" est
        ajoutee dans le course_bound_rule cote responder.py.
        """
        ...
        log.info(
            "🔍 personalization | style=%s pace=%s depth=%s tone=%s | "
            "avg_rt=%.1fs confusion_rate=%.2f difficulty=%s",
            ctx.learning_style, ctx.pace, ctx.explanation_depth, ctx.tone,
            ctx.avg_response_time_s, ctx.confusion_rate,
            ctx.preferred_difficulty,
        )
        ...
        if lang == "fr":
            return (
                ...
                f"Style cognitif a respecter : {composed}\n"
                "<<FIN_INSTRUCTION_INTERNE>>\n\n"
            )
        return (
            "<<INTERNAL_INSTRUCTION_DO_NOT_MENTION_IN_ANSWER>>\n"
            f"Cognitive style to apply: {composed}\n"
