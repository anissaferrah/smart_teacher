...
async def get_redis() -> aioredis.Redis:
    ...
    if _redis is None:
        ...
        log.info(f"🟢 REDIS connect | {host}:{port} | decode_responses=True")
    return _redis
...
VALID_TRANSITIONS: dict[DialogState, list[DialogState]] = {
    ...
    # WAITING -> PROCESSING is added : when the student is paused
    # (WAITING) and asks a real question, the FSM must allow the
    # direct jump to PROCESSING. Without it the WS handler crashed
    # with "Transition invalide : WAITING -> PROCESSING" because the
    # legacy chain WAITING -> LISTENING -> PROCESSING required two
    # state writes that the handler doesn't perform.
    DialogState.WAITING:    [DialogState.PRESENTING, DialogState.LISTENING, DialogState.PROCESSING, DialogState.IDLE, DialogState.CLARIFICATION],
...
class SessionContext:
    ...
    # Continuous fused confusion score from the last detection. Read
    # by the adaptive TTS rate logic — when this stays above
    # CONFUSION_TTS_SLOWDOWN_THRESHOLD across recent turns, the next
    # narration's speech_rate is reduced. EWMA-style smoothing lives
    # in the WS closure to avoid one-off spikes triggering slowdown.
    last_confusion_score:    float = 0.0
    # Per-session count of consecutive "clean" (low-confusion) turns.
    # Used to RESET the TTS slowdown after the student catches up.
...
class DialogueManager:
    ...
    async def _save(self, ctx: SessionContext) -> None:
        ...
        payload = ctx.to_json()
        _t0 = time.time()
        await r.setex(key, SESSION_TTL, payload)
        log.debug(
            f"🟢 REDIS SETEX | key={key} ttl={SESSION_TTL}s size={len(payload)}B "
            f"state={ctx.state} took={(time.time()-_t0)*1000:.1f}ms"
        )
    ...
    async def _load(self, session_id: str) -> Optional[SessionContext]:
        ...
        if not data:
            log.debug(f"🟢 REDIS GET MISS | key={key} took={elapsed:.1f}ms")
            return None
        log.debug(f"🟢 REDIS GET HIT | key={key} size={len(data)}B took={elapsed:.1f}ms")
    ...
    async def _delete(self, session_id: str) -> None:
        ...
        _t0 = time.time()
        await r.delete(key)
        log.info(f"🟢 REDIS DEL | key={key} took={(time.time()-_t0)*1000:.1f}ms")
    ...
    async def pause_session(
        ...
        # the caller didn't supply new values — but ONLY when the student
        # is still on the SAME slide. Otherwise we'd carry forward the
        # OLD slide's content into the new pause, and the Q&A handler
        # would feed the LLM the wrong slide as ground truth (operator-
        # visible bug : student paused on "Définitions et terminologie"
        # but the responder received the cover page text and answered
        # with hallucinated definitions like "RI = Research Interface").
        ...
        # Last-resort: if still no slide_path/content (caller didn't pass
        # it AND prior paused state was for a DIFFERENT slide), derive
        # both from course_id + slide_id. Most callers don't pass these
        # explicitly — this single DB lookup keeps the Q&A vision path
        # working without forcing every pause caller to thread them
        # through. Failure is silent (vision will just be skipped).
            ...
            try:
                ...
                if len(parts) >= 3:
                    ...
                    if slide_ctx:
                        ...
                        log.info(
                            "📑 pause_session slide_lookup | course=%s slide=%s | "
                            "path=%s content_len=%d (refreshed for new slide)",
        ...
        log.info(
            f"⏸️  [{session_id[:8]}] PAUSE_SESSION | slide={slide_id or 'unknown'} | "
            f"cursor={_eff_cursor}/{_text_len} ({_progress:.1f}%) | char_offset={char_offset} | "
            f"idea={effective_idea_id or '-'} | "
            f"timestamp={_dt.utcfromtimestamp(_pause_ts).isoformat()} (ts={_pause_ts:.3f}) | "
            f"interruptions_total={ctx.interruptions}"
        )
        return ctx
    ...
    async def resume_session(self, session_id: str) -> Optional[SessionContext]:
        ...
        ctx = await self._load(session_id)
        if not ctx:
            log.warning(f"[{session_id[:8]}] ▶️  RESUME_SESSION FAIL | no ctx loaded")
            return None

        if not ctx.paused_state.get("is_paused"):
            log.warning(f"[{session_id[:8]}] ▶️  RESUME_SESSION SKIP | session is not paused")
            return ctx
        ...
        await self._save(ctx)
        log.info(
            f"▶️  [{session_id[:8]}] RESUME_SESSION | slide={_slide_id} | "
            f"cursor={offset}/{_text_len} ({_progress:.1f}%) | "
            f"pause_duration={_pause_dur:.1f}s | "
            f"resume_at={_dt.utcfromtimestamp(_now).isoformat()}"
        )
        return ctx
    ...
    async def save_position(self, session_id: str, char_pos: int) -> None:
        ...
        if ctx:
            ...
            await self._save(ctx)
            _text_len = ctx.paused_state.get("presentation_text_len", 0)
            log.debug(
                f"[{session_id[:8]}] 💾 save_position | {_prev} → {char_pos}"
                + (f" (+{char_pos - _prev}ch) of {_text_len}" if _text_len else "")
                + (" + snapshot updated" if needs_snapshot_update else "")
            )
    ...
    async def save_presentation_snapshot(
        ...
        log.info(
            f"🟢 REDIS SETEX snapshot | key=presentation:snapshot:{session_id[:8]}:{slide_id} | "
            f"size={len(body)}B ttl={PRESENTATION_SNAPSHOT_TTL}s | "
            f"text_len={len(presentation_text or '')} cursor={presentation_cursor} | "
            f"took={(time.time()-_t0)*1000:.1f}ms"
        )
    ...
    async def load_presentation_snapshot(self, session_id: str, slide_id: str) -> Optional[dict]:
        ...
        if not raw:
            log.info(
                f"🟢 REDIS GET MISS snapshot | key=presentation:snapshot:{session_id[:8]}:{slide_id} | took={elapsed:.1f}ms"
            )
            return None
        try:
            data = json.loads(raw)
            if isinstance(data, dict):
                log.info(
                    f"🟢 REDIS GET HIT snapshot | key=presentation:snapshot:{session_id[:8]}:{slide_id} | "
                    f"size={len(raw)}B text_len={data.get('presentation_text_len', '?')} "
                    f"cursor={data.get('presentation_cursor', '?')} | took={elapsed:.1f}ms"
                )
                return data
        except Exception as exc:
            log.warning(f"🟢 REDIS GET PARSE_ERR snapshot | err={exc}")
        return None
    ...
    async def save_tts_phrase_cache(
        ...
        *,
        ...
        r = await get_redis()
        _t0 = time.time()
        await r.setex(cache_key, TTS_CACHE_TTL, body)
        log.info(
            f"🟢 REDIS SETEX tts_phrase | key={cache_key[:30]}... | "
            f"size={len(body)}B ttl={TTS_CACHE_TTL}s | "
            f"audio={len(audio_bytes)}B chars={len(text)} {provider}/{voice_name} {language} {rate} | "
            f"took={(time.time()-_t0)*1000:.1f}ms"
        )
        return cache_key
    ...
    async def load_tts_phrase_cache(
        ...
        *,
        ...
        elapsed = (time.time() - _t0) * 1000
        if not raw:
            log.debug(f"🟢 REDIS GET MISS tts_phrase | key={cache_key[:30]}... | chars={len(text)} took={elapsed:.1f}ms")
            return None
        ...
        try:
            ...
            log.info(
                f"🟢 REDIS GET HIT tts_phrase | key={cache_key[:30]}... | "
                f"audio={len(audio_bytes)}B chars={len(text)} | took={elapsed:.1f}ms (saved a TTS call)"
            )
            return {
                **data,
            ...
            log.warning(f"🟢 REDIS GET PARSE_ERR tts_phrase | err={exc}")
            return None
    ...
    async def detect_and_track_confusion(
        ...
        prosody: dict = None,               # NOW USED via fusion module
        on_state_change=None,
    ) -> tuple[bool, str, str, int]:
        """SIGHT + prosody fused confusion detection + Redis bookkeeping.

        Returns ``(is_confused, reason, q_hash, confusion_count)`` where
        ``reason`` is one of ``"sight_model"``, ``"prosody_strong"``,
        ``"fused_high"`` or the empty string.

        # Why a fusion now (and why the old layers stayed dead)

        Previously this method had 8 stacked detection layers (keywords,
        hash repetition, similarity-based, etc.) all OR-ed. The system
        was too sensitive — students got reformulations they didn't
        ask for. We collapsed it to SIGHT-only to recover precision.

        We now PRINCIPLY add ONE more signal — prosody — via
        ``pedagogy.confusion.fusion``. Prosody is :
          - vocal-only (orthogonal to text), so it adds genuinely new info
          - extracted from already-running ``audio.transcriber.extract_prosody``
          - weighted at 30 % (vs 70 % for SIGHT) so a clean text isn't
            over-ridden by vocal hesitation alone
        The behaviour-based layers (keyword lists, repetition heuristics,
        adaptive thresholds) stay dead — they had no calibration data.

        ``history`` and ``brain`` remain in the signature for back-compat
        but are still unused. Wire them back only with calibration data.
        """
        import hashlib, re, time

        start_ts = time.time()
        # Step 1 : SIGHT (text-only) — primary signal, calibrated
        sight_confused, sight_reason = self.detect_confusion(question_text, language)
        # The legacy ``detect_confusion`` returns (bool, reason). Map to
        # a [0, 1] score : 0.85 when SIGHT fires (above its own threshold,
        # high confidence), 0.10 when it doesn't (low but not zero so
        # prosody can still tip the fused score in extreme cases).
        sight_score = 0.85 if sight_confused else 0.10

        # Step 2 : fuse SIGHT + prosody
        from pedagogy.confusion.fusion import (
            ConfusionSignals, fuse_confusion_signals,
        )
        fused = fuse_confusion_signals(ConfusionSignals(
            text_score=sight_score,
            prosody_dict=prosody,
        ))
        # Decision : the fused score crosses a threshold matching where
        # the legacy SIGHT-only check would have fired (0.5). Above
        # that we flag confused ; the reason string tells the operator
        # which signal contributed (see fusion.reason).
        is_confused = fused.score >= 0.5
        if is_confused:
            if sight_confused and "prosody" in fused.contributors and fused.prosody_score >= 0.7:
                reason = "fused_text_and_prosody"
            elif sight_confused:
                reason = "sight_model"
            elif fused.prosody_score >= 0.7:
                reason = "prosody_strong"
            else:
                reason = "fused_high"
        else:
            reason = ""
        ...
        ctx.last_confusion_score = float(fused.score)
        if is_confused:
            ctx.confusion_count += 1
            ctx.consecutive_clean_turns = 0
        else:
            ctx.consecutive_clean_turns = (ctx.consecutive_clean_turns or 0) + 1
        await self._save(ctx)
        ...
        # Detailed fusion log — operator can see the breakdown of what
        # contributed (text vs prosody) and why the binary decision fired.
        log.info(
            f"[{session_id[:8]}] 🤔 confusion fusion | sight={sight_score:.2f} "
            f"({'fires' if sight_confused else 'clean'}) | "
            f"prosody={fused.prosody_score:.2f} ({len((prosody or {}).get('markers', []))} markers) "
            f"| fused={fused.score:.2f} → "
            f"{'CONFUSED' if is_confused else 'clean'} (reason={reason or 'none'}, {duration_ms}ms)"
        )

        if is_confused:
            log.info(
                f"[{session_id[:8]}] 🤔 confusion total this session: {ctx.confusion_count}"
            )
