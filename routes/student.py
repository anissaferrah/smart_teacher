...
async def submit_vark_responses(
    ...
    from database.models import StudentProfile, VarkResponse
    ...
    async with AsyncSessionLocal() as db:
        ...
        # Persist the RAW response in vark_responses too. The aggregated
        # ``vark_self_report`` above is convenient (it's what the
        # Bayesian seed uses) but it loses the per-question detail.
        # The raw row lets us recompute the posterior retroactively if
        # the scoring rule changes, and lets aggregate cohort analyses
        # ("VARK distribution for engineering students") pull straight
        # from the source of truth without reconstructing it from the
        # JSON bag inside StudentProfile.preferences.
        try:
            db.add(VarkResponse(
                student_id=sid,
                course_id=None,        # cross-course self-report
                responses=responses or {},
            ))
        except Exception as exc:                                            # noqa: BLE001
            # Persistence is best-effort ; the Bayesian seed update
            # below is the load-bearing path.
            log.warning("vark : raw row persist failed (%s)", exc)
...
async def get_student_profile_full(student_id: str):
    ...
    try:
        async with AsyncSessionLocal() as db:
            ...
            # ── Consolidated StudentModel snapshot ──────────────────────
            # Pulls from MasteryRepo + ProfileManager + Bayes posterior +
            # PersonalizationContext in parallel and exposes one frozen
            # view. Same structure consumers (responder, planner, bandit)
            # use at runtime, so what the student sees on /me/profile is
            # exactly what shapes their Q&A experience. Falls back to
            # safe defaults when a source is unreachable; ``ok``/``sources_failed``
            # surface that to the operator/UI.
            from pedagogy.student_model import StudentModel
            student_model = await StudentModel.fetch(
                student_id=str(sid),
                course_id=None,                # cross-course view on /me/profile
                idea_ids=[
                    str(m.idea_id) for m in mastery_rows
                    if getattr(m, "idea_id", None) is not None
                ],
            )
            ...
            return {
                ...
                # New unified student-state snapshot. UI can pick this
                # up directly via `/student/me/profile` → response.student_model.
...
@router.get("/student/me/reviews-due")
async def get_my_reviews_due(
    course_id: str | None = None,
    limit: int = 20,
    user: dict = Depends(get_current_user),
):
    """Same as ``/student/{id}/reviews-due`` but auth-scoped (uses the
    JWT bearer's student_id) so the FE doesn't need to know its own
    UUID. Mirrors the ``/student/me/profile`` pattern."""
    from pedagogy.review_scheduler import ReviewScheduler
