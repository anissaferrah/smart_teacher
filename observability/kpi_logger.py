...
class KPITracker:
    ...
    def mark_tts_stopped(self, session_id: str, turn_id: int = 0) -> Optional[float]:
        ...
        # Sanity guard against stale start times. When ``mark_tts_stopped``
        # was missed on a previous turn (e.g. cancellation thrown midway,
        # session crashed before the metric was recorded), the start_time
        # entry can survive in ``_t_interrupt_start`` indefinitely. A
        # later turn re-using the same (session_id, turn_id) key would
        # then compute a delta of minutes/hours — meaningless as a KPI
        # signal, only useful as a tell that the dict has stale state.
        # Threshold = 60s (orders of magnitude above the 500ms target).
        # Above that we drop the measurement, log at INFO so the operator
        # sees the cleanup, and clear any other stale entries for this
        # session to keep the dict bounded.
        _STALE_THRESHOLD_S = 60.0
        if latency > _STALE_THRESHOLD_S:
            log.info(
                f"KPI interrupt : dropping stale start_time "
                f"({latency:.1f}s > {_STALE_THRESHOLD_S}s) for {session_id[:8]} "
                f"— purging session entries"
            )
            # Drop every other stale entry for this session — they're
            # all from before the orphaned start and would all give bogus
            # readings if hit later.
            stale_keys = [k for k in self._t_interrupt_start
                          if k[0] == session_id]
            for k in stale_keys:
                self._t_interrupt_start.pop(k, None)
            return None
