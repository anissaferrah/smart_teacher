        ...
        *,
        ...
        try:
            ...
            try:
                ...
                qa_state = {
                    "session_id": session_id,
                    "course_id": (question_ctx.course_id if question_ctx else "") or "",
            ...
            tts_rate = await get_edge_tts_rate_with_bandit(
                session_id, bandit_speech_rate=bandit_speech_rate,
            )
        ...
        while True:
            ...
            # ── present_section — présenter une section de cours ─────
                ...
                if reuse_cached_narration:
                    cache_source = "memory" if current_presentation_key == requested_slide_key and bool(current_presentation_text) else "redis"
                    log.info(
                        f"[{session_id[:8]}] 💾 Narration cache HIT (source={cache_source}, slide={requested_slide_id}) → 0 LLM call"
                    )
                ...
                async def run_presentation(
                    ...
                    try:
                        ...
                        # ✨ Forward-skip slicing : skip past the partial sentence
                        # the student was hearing so we don't repeat content. The
                        # optional pedagogical recap ("We were just covering X —
                        # let's continue.") is built separately and played as a
                        # SEPARATE TTS chunk before the resumed narration so the
                        # cursor stays aligned with positions in narration_text.
                        resume_recap = ""
                        if effective_resume > 0:
                            from services.presentation import (
                                resume_at_clean_boundary,
                                build_resume_recap,
                            )
                            remaining_text = resume_at_clean_boundary(
                                narration_text, effective_resume, language=lang_ps,
                            )
                            new_start = len(narration_text) - len(remaining_text)
                            skipped = max(0, new_start - effective_resume)
                            resume_recap = build_resume_recap(
                                narration_text, effective_resume, language=lang_ps,
                            )
                            log.info(
                                f"[{session_id[:8]}] ↪️  Resume forward-skip: "
                                f"raw_offset={effective_resume} → text starts at char {new_start} "
                                f"(skipped {skipped} chars of partial sentence; "
                                f"recap={'on' if resume_recap else 'off'})"
                            )
                        else:
